...
import (
	...
	"github.com/gcash/cbdc/rpcclient"
...
// Harness fully encapsulates an active cbdc process to provide a unified
// platform for creating rpc driven integration tests involving cbdc. The
// active cbdc node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active cbdc process is fully
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the cbdc node itself. This spawns a new process which will be
	// managed
	...
	// Ensure cbdc properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created cbdc
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "cbdc", "rpctest")
