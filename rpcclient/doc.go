...
Bitcoin RPC server that uses a cbdc/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with cbdc (https://github.com/gcash/cbdc),
bchwallet (https://github.com/gcash/bchwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, cbdc and
...
cbdc or bchwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by cbdc and
...
All notifications provided by cbdc require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by cbdc (and
...
Also, it is important to realize that cbdc intentionally separates the wallet
functionality into a separate process named bchwallet.  This means if you are
connected to the cbdc RPC server directly, only the RPCs which are related to
